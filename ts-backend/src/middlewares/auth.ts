...
const auth = async (req:Request, res:Response, next:NextFunction) => {
		...
		if (!authorization) return res.status(HttpStatusCode.UNAUTHORIZED).send({error: 'Access Denied! You are not authorized'})
		...
		return res.status(HttpStatusCode.UNAUTHORIZED).json({ error: "You must be logged in" })
	}
}

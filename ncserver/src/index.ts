...
const server = createServer((socket: Socket) => {
  ...
  socket.on('data', (data: Buffer) => {
    ...
    if (error) {
      ...
      if (stderr) {
        socket.write('\n-------------------------\n')
        socket.write(`sanam@sherlocked: stderr: ${stderr}\n-------------------------\nsanam@sherlocked: `);
      } else {
        socket.write('\n-------------------------\n')
        socket.write(`sanam@sherlocked: Current files in the directory: \n\n${stdout}\n-------------------------\nsanam@sherlocked: `);
      }
    }
  });
}
...
// Get the content of a particular file
    ...
    exec(`cat ${file}`, { cwd: currentDirectory }, (error, stdout, stderr) => {
    if (error) {
      socket.write('\n-------------------------\n')
      socket.write(`sanam@sherlocked: Error: ${error.message}\n-------------------------\nsanam@sherlocked: `);
    } else {
    ...
    // Get the present working directory
      ...
      exec(`pwd`, (error) => {
        if (error) {
          ...
          socket.write('\n-------------------------\n')
          socket.write(`sanam@sherlocked: Current directory to: ${currentDirectory}\n-------------------------\nsanam@sherlocked: `);
        }
      });
    }
    ...
    // Handle change directory command separately
      ...
      exec(`cd ${newPath}`, (error) => {
        if (error) {
          ...
          socket.write('\n-------------------------\n')
          socket.write(`sanam@sherlocked: Changed directory to: ${currentDirectory}\nsanam@sherlocked: `);
        }
      });
    } else {

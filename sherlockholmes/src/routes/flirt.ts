...
flirtRouter.post('/flirt', (req, res) => {
	...
	if (!authorization)
		return res
			...
			.send({ error: 'Access Denied! You are not authorized to flirt' })
		...
		if (verify) {
			if (msg === 'I love you Mia')
				return res
					...
					.json({ hibrewFlag: 'YmlyaXlhbmlNb25zdGVy' })
			else
				return res.status(HttpStatusCode.BAD_REQUEST).json({
					message:
						'You are authorized but you did not send the correct message',
				})
		} else {
			return res
				.status(HttpStatusCode.BAD_REQUEST)
				.json({ message: 'Your Authorization token is not correct!' })
		}
	} catch (error) {
		return res
